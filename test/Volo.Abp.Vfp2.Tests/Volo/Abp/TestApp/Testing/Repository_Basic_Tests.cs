...
namespace Volo.Abp.TestApp.Testing
{
        ...
        where TStartupModule : IAbpModule
    {
        ...
        {
            var person = await PersonRepository.FindAsync(Guid.NewGuid().ToString());
            person.ShouldBeNull();
        }
        ...
        {
            await PersonRepository.DeleteAsync(TestDataBuilder.UserDouglasId.ToString());

            (await PersonRepository.FindAsync(TestDataBuilder.UserDouglasId.ToString())).ShouldBeNull();
        }
        ...
        public virtual async Task InsertAsync()
        {
            ...
            await PersonRepository.InsertAsync(new Person(personId.ToString(), "Adam", 42));

            var person = await PersonRepository.FindAsync(personId.ToString());
